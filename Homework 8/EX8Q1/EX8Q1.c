// EX8Q1.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
//{
    /* read in the file path from the CLI */
   /* size_t filePathLength = strlen(argv[1]);

    char *filePath = (char *)malloc(sizeof(char) * filePathLength);
    if (filePath)
    {
        strcpy_s(filePath, filePathLength, argv[1]);
    }

    uLong cmp_len = mz_compressBound(filePathLength);

    if (IsZipFile(filePath))
    {
        mz_uncompress(CreateUnzippedFileName(filePath), )
    }*/
